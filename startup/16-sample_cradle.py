...
class Cradle(PseudoPositioner):
    ...
    yout = Cpt(EpicsMotor, 'LinXS}Mtr')
    yin  = Cpt(EpicsMotor, 'LinY}Mtr')
    xout = Cpt(EpicsMotor, 'LinS}Mtr')
...
xafs_linxs.user_offset.put(251.362)
...
def exercise_cradle(n=5):
    ...
    yield from mvr(cradle.x,    n)
    yield from mvr(cradle.x, -1*n)
