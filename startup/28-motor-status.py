...
def motor_status():
    ...
    text = 'Energy = %.1f   reflection = Si(%s)   mode = %s\n' % (dcm.energy.readback.value,
                                                                  dcm.crystal, dcm.mode)
    text += '\tBragg = %8.5f   2nd Xtal Perp = %7.4f   2nd Xtal Para = %8.4f\n\n' % \
            (dcm.bragg.user_readback.value,
             dcm.perp.user_readback.value,
             dcm.para.user_readback.value)

    text += 'M2\tvertical = %7.3f mm\t\tYU  = %7.3f\n'   % (m2.vertical.readback.value, m2.yu.user_readback.value)
    ...
    text += 'Slits3:\tvertical   size   = %7.3f mm\t\tTop      = %7.3f\n' % \
            (slits3.vsize.readback.value,   slits3.top.user_readback.value)
    text += '\tvertical   center = %7.3f mm\t\tBottom   = %7.3f\n' % \
            (slits3.vcenter.readback.value, slits3.bottom.user_readback.value)
    text += '\thorizontal size   = %7.3f mm\t\tOutboard = %7.3f\n' % \
            (slits3.hsize.readback.value,   slits3.outboard.user_readback.value)
    text += '\thorizontal center = %7.3f mm\t\tInboard  = %7.3f\n\n' % \
            (slits3.hcenter.readback.value, slits3.inboard.user_readback.value)

    text += 'DM3_BCT: %7.3f mm\n\n' % dm3_bct.user_readback.value

    text += 'XAFS table:\n\tvertical = %7.3f mm\t\tYU  = %7.3f\n'   % \
            (xafs_table.vertical.readback.value, xafs_table.yu.user_readback.value)
    text += '\tpitch    = %7.3f mrad\t\tYDO = %7.3f\n' % \
            (xafs_table.pitch.readback.value,    xafs_table.ydo.user_readback.value)
    text += '\troll     = %7.3f mrad\t\tYDI = %7.3f\n\n' % \
            (xafs_table.roll.readback.value,     xafs_table.ydi.user_readback.value)

    text += 'XAFS stages:\n'
    text += '\t    x       y      roll    pitch    xs       rh       rb       rs\n'
    text += '\t%7.3f %7.3f %7.3f %7.3f %7.3f %8.3f %8.3f %8.3f\n' % \
