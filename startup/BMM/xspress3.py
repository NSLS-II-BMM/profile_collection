...
from nslsii.areadetector.xspress3 import Xspress3FileStore, Xspress3Trigger
...
from databroker.assets.handlers import XS3_XRF_DATA_KEY, HandlerBase, Xspress3HDF5Handler
...
class BMMXspress3DetectorBase(Xspress3Trigger, Xspress3Detector):
    ...
    def __init__(self, prefix, *, configuration_attrs=None, read_attrs=None, **kwargs):
        ...
        self.slots = ["Ti", "V", "Cr", "Mn", "Fe", "Co", "Ni", "Cu", "Zn", "Ge", "As", "Br", "Nb", "Mo", None, "OCR"]
        self.restart()
    ...
    def set_roi(self, mcaroi, name="OCR", min_x=1, size_x=4095):
        ...
        #     name.decode('utf8')
        mcaroi.configure_mcaroi(roi_name=name, min_x=min_x, size_x=size_x)
        mcaroi.name = name
    ...
    def roi_details(self):
        ...
        for i, el in enumerate(self.slots):
            ...
            if el is None:
                ...
                print(
                    go_msg(template % (i + 1, el.capitalize(), this.min_x.get(), this.min_x.get() + this.size_x.get()))
                )
            else:
                print(template % (i + 1, el.capitalize(), this.min_x.get(), this.min_x.get() + this.size_x.get()))
