from ophyd import Component as Cpt, EpicsSignal, Signal
...
class Vacuum(Device):
        ...
        def _pressure(self):
                if float(self.pressure.value) > 1e-6:
                        return('\x1b[01m\x1b[31m' + self.pressure.value + '\x1b[0m')
                if float(self.pressure.value) > 1e-8:
                        return('\x1b[01m\x1b[33m' + self.pressure.value + '\x1b[0m')
...
class GateValve(Device):
        ...
        def _state(self):
                if self.state.value == 0:
                        return('\x1b[01m\x1b[31m' + 'closed' + '\x1b[0m')
                return('open  ')
...
class Thermocouple(Device):
        ...
        def _state(self):
                ...
                if self.temperature.value > self.alarm.value:
                        return('\x1b[01m\x1b[31m' + t + '\x1b[0m')
                if self.temperature.value > self.warning.value:
                        return('\x1b[01m\x1b[33m' + t + '\x1b[0m')
                return(t)
